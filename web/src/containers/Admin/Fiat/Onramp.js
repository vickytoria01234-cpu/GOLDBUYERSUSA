...
const Onramp = ({
	...
	const activeItem =
		coinData.find((item) => item?.symbol === activeRampSymbol) || coinData[0];

	const renderCoinConfig = (item) => {
		if (!item) {
			return null;
		}
		return (
			<div>
				<div className="paymentbox2">
					...
					<Button
						...
						disabled={
							!user_payments || !Object.keys(user_payments).length || isDisable
						}
					>
						Add on-ramp
					</Button>
