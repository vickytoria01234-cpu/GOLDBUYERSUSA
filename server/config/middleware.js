...
const pickPasswordAccount = (req) => {
	const swaggerVal = req.swagger && req.swagger.params && req.swagger.params.data
...
// Reset-password mint (GET /reset-password) carries the target identifier in
// the query string (`value`, or the deprecated `email`/`phone_number`). Keying
// the limiter on it stops an attacker spamming reset codes/emails at a single
// victim from rotating IPs. The consume (POST) carries only the code, so it is
// capped per-IP instead (see below).
const pickResetAccount = (req) => {
	const q = (req.swagger && req.swagger.params) || {};
	const pick = (name) => (q[name] && q[name].value) || (req.query && req.query[name]) || '';
	const value = pick('value').toString().trim();
	const email = (pick('email') || (value.includes('@') ? value : '')).toString().trim().toLowerCase();
	const phone = (pick('phone_number') || (!value.includes('@') ? value : '')).toString().trim();
	if (email) return `email:${email}`;
	if (phone) return `phone:${phone}`;
	return null;
};
...
const rateLimitMiddleware = (app) => {
	...
	// Per-account limiter for the reset-password mint (GET) keyed by email or
	// phone_number so an attacker cannot spam reset codes at a single victim
	// from rotating IPs. Requests with no identifier fall into an IP-scoped
	// 'unknown' bucket.
	limiter({
		path: '/v2/reset-password',
		method: 'get',
		total: 4,
		expire: 1000 * 60 * 2,
		lookup: accountOrIpUnknownLookup(pickResetAccount),
		onRateLimited: function (req, res, next) {
			logger.verbose('config/middleware/rateLimitMiddleware', 'abuse', 'get reset-password account');
			return res.status(429).json({ message: 'Too many requests. Your account is blocked for 2 minutes' });
		}
	});

	// Per-account limiter for /v2/password keyed by email or phone_number.
	// Requests with neither fall into an IP-scoped 'unknown' bucket.
	limiter({
		path: '/v2/password',
		method: 'post',
		total: 4,
		expire: 1000 * 60 * 2,
		lookup: accountOrIpUnknownLookup(pickPasswordAccount),
		onRateLimited: function (req, res, next) {
			logger.verbose('config/middleware/rateLimitMiddleware', 'abuse', 'post password account');
			return res.status(429).json({ message: 'Too many requests. Your account is blocked for 2 minutes' });
		}
	});
	...
	// /user/addressbook/whitelist requests trusting (whitelisting) an address.
	// Each call costs a real outbound email/SMS verification code, so cap it
	// tightly per-user (with IP fallback) to prevent spamming the owner and
	// flooding redis with pending tokens.
	limiter({
		path: '/v2/user/addressbook/whitelist',
		method: 'post',
		total: 5,
		expire: 1000 * 60 * 2,
		lookup: authOrIpLookup,
		onRateLimited: function (req, res, next) {
			logger.verbose('config/middleware/rateLimitMiddleware', 'abuse', 'addressbook-whitelist-request');
			return res.status(429).json({ message: 'Too many requests. Your account is blocked for 2 minutes' });
		}
	});

	// /user/addressbook/whitelist/confirm submits the emailed verification code
	// (as short as a 6-digit v4 code). The tools layer namespaces the token by
	// user_id, but this transport-level cap is defense in depth against a stolen
	// session brute-forcing the code.
	limiter({
		path: '/v2/user/addressbook/whitelist/confirm',
		method: 'post',
		total: 5,
		expire: 1000 * 60 * 2,
		lookup: authOrIpLookup,
		onRateLimited: function (req, res, next) {
			logger.verbose('config/middleware/rateLimitMiddleware', 'abuse', 'addressbook-whitelist-confirm');
			return res.status(429).json({ message: 'Too many requests. Your account is blocked for 2 minutes' });
		}
	});
	...
	// /user/freeze-account is unauthenticated and consumes a short (6-digit)
	// freeze token via a direct Redis lookup with no per-token fail-counter,
	// so brute-forcing the token across requests is the main risk. Cap per-IP
	// as a transport-level limit (the body carries only the token, no account
	// to key on).
	limiter({
		path: '/v2/user/freeze-account',
		method: 'post',
		total: 5,
		expire: 1000 * 60 * 2,
		lookup: 'headers.x-forwarded-for',
		onRateLimited: function (req, res, next) {
			logger.verbose('config/middleware/rateLimitMiddleware', 'abuse', 'freeze-account');
			return res.status(429).json({ message: 'Too many requests. Your account is blocked for 2 minutes' });
		}
	});

	// /login/passkey-auth-options is unauthenticated, takes an email, runs
	// a DB lookup, and writes a Redis challenge per call. Without a cap
	// an attacker can spam Redis, enumerate emails by timing/response
	// shape, and freely cycle the challenge keyspace. Per-IP only — there
	// is no stable principal we can key on for unauthenticated callers.
	limiter({
		path: '/v2/login/passkey-auth-options',
		method: 'get',
		total: 10,
		expire: 1000 * 60 * 2,
		lookup: 'headers.x-forwarded-for',
		onRateLimited: function (req, res, next) {
			logger.verbose('config/middleware/rateLimitMiddleware', 'abuse', 'passkey-auth-options');
			return res.status(429).json({ message: 'Too many requests. Your account is blocked for 2 minutes' });
		}
	});

	// /login/verify-passkey issues a session JWT on success and, when the
	// suspicious-login feature is on, sends a verification email per call.
	// Without a cap that branch is an outbound-email amplifier. Per-IP cap
	// matches the unauthenticated nature of the route.
	limiter({
		path: '/v2/login/verify-passkey',
		method: 'post',
		total: 10,
		expire: 1000 * 60 * 2,
		lookup: 'headers.x-forwarded-for',
		onRateLimited: function (req, res, next) {
			logger.verbose('config/middleware/rateLimitMiddleware', 'abuse', 'verify-passkey');
			return res.status(429).json({ message: 'Too many requests. Your account is blocked for 2 minutes' });
		}
	});

	// /subaccount/auth and /sharedaccount/auth mint full bearer JWTs that
	// are equivalent in power to a /login token, with no second factor and
	// no captcha. They require a master/shared bearer to call, so we key
	// by authenticated user id (with IP fallback). Without this cap, a
	// leaked master token can be expanded into independent JWTs for every
	// linked account at line speed.
	limiter({
		path: '/v2/subaccount/auth',
		method: 'post',
		total: 10,
		expire: 1000 * 60 * 2,
		lookup: authOrIpLookup,
		onRateLimited: function (req, res, next) {
			logger.verbose('config/middleware/rateLimitMiddleware', 'abuse', 'subaccount-auth');
			return res.status(429).json({ message: 'Too many requests. Your account is blocked for 2 minutes' });
		}
	});

	limiter({
		path: '/v2/sharedaccount/auth',
		method: 'post',
		total: 10,
		expire: 1000 * 60 * 2,
		lookup: authOrIpLookup,
		onRateLimited: function (req, res, next) {
			logger.verbose('config/middleware/rateLimitMiddleware', 'abuse', 'sharedaccount-auth');
			return res.status(429).json({ message: 'Too many requests. Your account is blocked for 2 minutes' });
		}
	});
};
