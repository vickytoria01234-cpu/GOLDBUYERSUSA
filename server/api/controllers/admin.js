...
const getUsersAdmin = (req, res) => {
	...
	toolsLib.user.getAllUsersAdmin({
		...
		.then(async (data) => {
			// When viewing a single user, attach company info for company accounts
			if ((id.value || user_id.value) && data?.data?.length === 1 && data.data[0].is_company) {
				data.data[0].company = await toolsLib.user.getUserCompany(data.data[0].id);
			}
			return res.json(data);
		})
		.catch((err) => {
...
const getCompaniesAdmin = (req, res) => {
	loggerAdmin.verbose(req.uuid, 'controllers/admin/getCompaniesAdmin/auth', req.auth);
	...
	if (order_by.value && typeof order_by.value !== 'string') {
		loggerAdmin.error(
			req.uuid,
			'controllers/admin/getCompaniesAdmin invalid order_by',
			order_by.value
		);
		return res.status(400).json({ message: 'Invalid order by' });
	}
	...
	toolsLib.user.getAllCompaniesAdmin({
		search: search.value,
		country_of_incorporation: country_of_incorporation.value,
		status: status.value,
		limit: limit.value,
		page: page.value,
		order_by: order_by.value,
		...
		.catch((err) => {
			loggerAdmin.error(req.uuid, 'controllers/admin/getCompaniesAdmin', err.message);
			const messageObj = errorMessageConverter(err, req?.auth?.sub?.lang);
			return res.status(err.statusCode || 400).json({ message: messageObj?.message, lang: messageObj?.lang, code: messageObj?.code });
		});
};
...
const putUserRole = (req, res) => {
	...
	toolsLib.user.updateUserRole(user_id, role, req.auth.sub.id, otp_code)
		.then((user) => {
			toolsLib.user.createAuditLog({ email: req?.auth?.sub?.email, session_id: req?.session_id }, req?.swagger?.apiPath, req?.swagger?.operationPath?.[2], { user_id, ...req?.swagger?.params?.data?.value });
...
const putMint = (req, res) => {
	...
	toolsLib.wallet.updatePendingMint(transaction_id, {
		...
		// operator clearing a non-travel_rule hold reason (blacklist/scorechain/kyt/
		// future). The network releases the deposit only if no other reason remains.
		clearHoldReason: clear_hold_reason,
		clearHoldSource: clear_hold_source,
		additionalHeaders: {
			'x-forwarded-for': req.headers['x-forwarded-for']
		}
	})
...
const reallocateDeposit = (req, res) => {
	loggerAdmin.verbose(
		req.uuid,
		'controllers/admin/reallocateDeposit auth',
		req.auth
	);

	const { transaction_id, address, receiver_id, description } = req.swagger.params.data.value;

	loggerAdmin.info(
		req.uuid,
		'controllers/admin/reallocateDeposit',
		'transaction_id',
		transaction_id,
		'address',
		address,
	...
	toolsLib.wallet.reallocateDepositByKitId(transaction_id, address, receiver_id, {
		description,
		additionalHeaders: {
			'x-forwarded-for': req.headers['x-forwarded-for']
		}
		...
		.then((data) => {
			...
			loggerAdmin.info(
				req.uuid,
				'controllers/admin/reallocateDeposit successful'
			);
			return res.json(data);
		})
		.catch((err) => {
			loggerAdmin.error(
				req.uuid,
				'controllers/admin/reallocateDeposit err',
				err
			);
			const messageObj = errorMessageConverter(err, req?.auth?.sub?.lang);
			return res.status(err.statusCode || 400).json({ message: messageObj?.message, lang: messageObj?.lang, code: messageObj?.code });
...
const getBlacklistedAddresses = (req, res) => {
	loggerAdmin.verbose(req.uuid, 'controllers/admin/getBlacklistedAddresses', req.auth.sub);

	const { limit, page, order_by, order, search } = req.swagger.params;

	toolsLib.wallet.getBlacklistedAddresses({
		limit: limit.value,
		page: page.value,
		order_by: order_by.value,
		order: order.value,
		...
		.catch((err) => {
			loggerAdmin.error(req.uuid, 'controllers/admin/getBlacklistedAddresses', err.message);
			const messageObj = errorMessageConverter(err, req?.auth?.sub?.lang);
			return res.status(err.statusCode || 400).json({ message: messageObj?.message, lang: messageObj?.lang, code: messageObj?.code });
		});
};

const postBlacklistedAddress = (req, res) => {
	loggerAdmin.verbose(req.uuid, 'controllers/admin/postBlacklistedAddress', req.auth.sub);

	const { address, network, label, reason } = req.swagger.params.data.value;

	loggerAdmin.info(req.uuid, 'controllers/admin/postBlacklistedAddress', 'address', address, 'network', network);

	toolsLib.wallet.createBlacklistedAddress({
		address,
		network,
		label,
		reason,
		...
		.catch((err) => {
			loggerAdmin.error(req.uuid, 'controllers/admin/postBlacklistedAddress', err.message);
			const messageObj = errorMessageConverter(err, req?.auth?.sub?.lang);
			return res.status(err.statusCode || 400).json({ message: messageObj?.message, lang: messageObj?.lang, code: messageObj?.code });
		});
};

const deleteBlacklistedAddress = (req, res) => {
	loggerAdmin.verbose(req.uuid, 'controllers/admin/deleteBlacklistedAddress', req.auth.sub);

	const id = req.swagger.params.id.value;

	loggerAdmin.info(req.uuid, 'controllers/admin/deleteBlacklistedAddress', 'id', id);

	toolsLib.wallet.deleteBlacklistedAddress(id)
		.then((result) => {
			toolsLib.user.createAuditLog({ email: req?.auth?.sub?.email, session_id: req?.session_id }, req?.swagger?.apiPath, req?.swagger?.operationPath?.[2], { id });
			return res.json(result);
		})
		.catch((err) => {
			loggerAdmin.error(req.uuid, 'controllers/admin/deleteBlacklistedAddress', err.message);
			const messageObj = errorMessageConverter(err, req?.auth?.sub?.lang);
			return res.status(err.statusCode || 400).json({ message: messageObj?.message, lang: messageObj?.lang, code: messageObj?.code });
		});
};
...
const putUserCompany = (req, res) => {
	loggerAdmin.verbose(
		req.uuid,
		'controllers/admin/putUserCompany auth',
		req.auth
	);
	...
	loggerAdmin.info(
		req.uuid,
		'controllers/admin/putUserCompany user_id:',
		user_id,
		'updateData:',
		updateData
	);
	...
	const auditInfo = { userEmail: req?.auth?.sub?.email, sessionId: req?.session_id, apiPath: req?.swagger?.apiPath, method: req?.swagger?.operationPath?.[2] };
	toolsLib.user.updateUserCompany(user_id, updateData, auditInfo)
		.then((data) => {
			return res.json(data);
		})
		.catch((err) => {
			loggerAdmin.error(
				req.uuid,
				'controllers/admin/putUserCompany err',
				err.message
			);
			const messageObj = errorMessageConverter(err, req?.auth?.sub?.lang);
			return res.status(err.statusCode || 400).json({ message: messageObj?.message, lang: messageObj?.lang, code: messageObj?.code });
...
const updateExchangeUserRole = (req, res) => {
	...
	loggerAdmin.verbose(req.uuid, 'controllers/admin/updateExchangeUserRole data', id, description, permissions, configs, color, restrictions);
