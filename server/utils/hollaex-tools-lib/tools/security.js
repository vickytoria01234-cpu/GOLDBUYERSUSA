...
// CF-IPCountry is only trusted when the operator explicitly enables it for
// deployments whose origin is protected by Cloudflare/proxy rules.
const getCountryFromHeaders = (headers = {}) => {
	if (!getKitConfig()?.cloudflare_ipcountry?.active) {
		return '';
	}
	const cf = headers?.['cf-ipcountry'];
	if (typeof cf === 'string' && cf && cf !== 'XX' && cf !== 'T1') {
		return cf.toUpperCase();
	}
	return '';
};

const getCountryFromIp = (ip, headers = {}) => {
...
const checkCaptcha = (captcha = '', remoteip = '', headers = {}) => {
	// `headers` is accepted for backward compatibility; previously it was
	// used to honour a `custom-device` bypass header, which has been removed
	// because any client (including non-browser callers reaching the API
	// through the open CORS surface) could trivially set it and skip
	// Turnstile entirely. The parameter is intentionally ignored.
	void headers;
...
const changeUserPassword = (email, oldPassword, newPassword, ip, domain, otpCode, version, verification_method) => {
	...
	return dbQuery.findOne('user', { where: { email } })
		...
		.then(async ([code, user]) => {
			const isShortCodeVersion = version === 'v3' || version === 'v4';
			if (isShortCodeVersion) {
			...
			await sendVerificationCode(user, {
				...
				verification_code: code,
				emailType: isShortCodeVersion
					? MAILTYPE.CHANGE_PASSWORD_CODE
					: MAILTYPE.CHANGE_PASSWORD,
				emailData: isShortCodeVersion
...
const setInitialUserPassword = (email, password) => {
	...
	return dbQuery.findOne('user', { where: { email: email } })
		.then((user) => {
			...
			if (getKitConfig().email_verification_required) {
				const isPhoneUser = user.meta && user.meta.phone_signup === true;
				if (isPhoneUser && !user.phone_number_verified) {
					throw new Error(USER_PHONE_NOT_VERIFIED);
				} else if (!isPhoneUser && !user.email_verified) {
...
const normalizeResetPasswordPhone = (raw) => {
...
const sendResetPasswordCode = (identifier, captcha, ip, domain, version, headers = {}, verification_method) => {
	if (typeof identifier !== 'string') {
		return reject(new Error(USER_NOT_FOUND));
	}

	const value = identifier.trim();
	const email = isEmail(value) ? value.toLowerCase() : null;
	const phone_number = email ? null : normalizeResetPasswordPhone(value);

	if (!email && !phone_number) {
		return reject(new Error(USER_NOT_FOUND));
	}

	return dbQuery.findOne('user', { where: email ? { email } : { phone_number } })
		.then((user) => {
			if (!user) {
				throw new Error(USER_NOT_FOUND);
			}
